...
def prepare_btn(btn, w=-1, h=-1):
    for state, color in COLOR_BG_BUTTONS:
        btn.modify_bg(state, Gdk.RGBA.parse(color))
    c = btn.get_child()
    if c is not None:
        for state, color in COLOR_FG_BUTTONS:
            c.modify_fg(state, Gdk.RGBA.parse(color))
    else:
        for state, color in COLOR_FG_BUTTONS:
            btn.modify_fg(state, Gdk.RGBA.parse(color))
    if w>0 or h>0:
...
class SliderPuzzleUI (Gtk.Table):
    ...
    def __init__(self, parent):
        ...
        self.btn_solve = prepare_btn(Gtk.Button(label=" "), 200)
        ...
        self.btn_shuffle = prepare_btn(Gtk.Button(label=" "), 200)
        ...
        self.btn_add = prepare_btn(Gtk.Button(label=" "), 200)
        ...
        # Control panel end
        panel.pack_start(control_panel, expand=True, fill=True, padding=0)
